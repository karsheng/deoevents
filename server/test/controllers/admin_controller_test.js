...
describe('Admin Controller', function(done) {
	...
	it('POST to /admin/interest creates a new interest', done => {
		request(app)
			.post('/admin/interest')
			.set('admin-authorization', adminToken)
			.send({ name: '5km' })
			.end((err, res) => {
				Interest.findOne({ name: '5km' })
				.then(interest => {
					assert(interest.name === '5km');
					done();
				});
			});
	});

	it('DELETE to /admin/interest/interest_id deletes an interest', done => {
		request(app)
			.delete(`/admin/interest/${interest._id}`)
			.set('admin-authorization', adminToken)
			.end((err, res) => {
				Interest.findOne({ name: '10km'})
				.then(result => {
					assert(result === null);
					done();
				});
